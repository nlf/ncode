...
ncode themes are JSON files that override any subset of the built-in
...
Missing values inherit from ncode's built-in default for the detected
terminal background.
...
saved in `$NCODE_HOME/config.json` and apply immediately. If the selected
file is deleted later, ncode resets the setting to the built-in auto
(default) theme.
...
missing, ncode applies `colors.dark` overrides on top of the built-in
...
- `background` — optional full-row TUI background. If missing, ncode uses the terminal's existing background. Experimental: terminal background colors can vary by emulator and scrollback behavior; for the most reliable result, change your terminal background color in your terminal settings instead.
...
- `spinner_messages` — list of messages; ncode picks one per turn.
...
ncode theme files are plain JSON, not JSONC. Validate before installing:
