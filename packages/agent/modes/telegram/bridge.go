...
type State struct {
	Running  bool
	Username string // bot username, e.g. "ncodebot"
...
func (b *Bridge) Start(parent context.Context) error {
	...
	if b.Config.BotToken == "" {
		b.mu.Unlock()
		return fmt.Errorf("no bot token configured; run `ncode telegram-bot setup` first")
	}
	b.mu.Unlock()
...
// side initiated the turn: TUI-originated turns get "ncode: " so the
// two-sided transcript reads naturally ("you: ..." / "ncode: ..."),
// while Telegram-originated turns send bare text (the user's own
// bubble is already on-screen, a "ncode: " prefix would just add
...
// OnUserTyped mirrors a message the user typed in the ncode TUI into
...
func (b *Bridge) handleUpdate(ctx context.Context, u Update) {
	...
	if paired == 0 {
		if isCommand && command == bot.CmdStart {
			...
			_ = b.Client.SendMessage(ctx, msg.Chat.ID,
				fmt.Sprintf("paired with @%s. messages you send here now mirror into the ncode tui.", msg.From.Username),
				msg.MessageID)
	...
	if isCommand {
		...
		case bot.CmdStart, bot.CmdHelp:
			_ = b.Client.SendMessage(ctx, msg.Chat.ID,
				"mirror is active. send me a message and it'll be forwarded to the ncode tui. commands: /status, /stop, or plain stop.",
				msg.MessageID)
		case bot.CmdStatus:
