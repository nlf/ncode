...
// spaces terminals commonly support. Most of ncode's theme still uses
...
// ANSI 256-color palette used by ncode. Defined as numeric codes so we
...
type Theme struct {
	...
	Assistant    int            // label color for the ncode role
	Tool         int
	ToolOut      int
	Error        int
	Warning      int
...
var Dark = Theme{
	...
	Assistant:         117,                  // bright cyan — the ncode label color
	Tool:              114,                  // green
	ToolOut:           245,
	Error:             203,
	Warning:           214,
...
// prefix in the chat ("▌ you", "▌ ncode") and as the editor prompt so
...
// TUI background. Empty means ncode should leave the terminal's
...
// ncode-blue gutter at the very left. The bar lives outside the bubble
...
func (t Theme) UserBubbleRow(content string, width int) string {
	...
	// so it matches the tool-box / app accent and reads as ncode's voice
