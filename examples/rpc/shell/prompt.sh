#!/usr/bin/env bash
# Minimal shell client for the `ncode rpc` JSON protocol.
#
# Usage:
...
# Pipe the command into ncode rpc; pipe its stdout through jq to react
# to events. The trailing `cat` keeps the input pipe open until the
# subprocess exits on its own (after `done`).
{
  if [ -n "${NCODE_RPC_TOKEN:-}" ]; then
    jq -nc --arg t "$NCODE_RPC_TOKEN" '{id:"0",type:"hello",token:$t}'
  fi
  echo "$cmd"
  # Block here so stdin stays open until ncode exits.
  cat
} | ncode rpc | while IFS= read -r line; do
