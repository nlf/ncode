"""Minimal Python client for the `ncode rpc` JSON protocol.

Usage:
    python ncode_client.py "fix the failing test"

Spawns `ncode rpc`, sends one prompt, prints assistant text as it streams,
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: ncode_client.py <prompt>", file=sys.stderr)
        return 2
    prompt = " ".join(sys.argv[1:])

    client = NcodeClient()
    try:
        token = os.environ.get("NCODE_RPC_TOKEN")
        if token:
