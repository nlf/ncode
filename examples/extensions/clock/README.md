# clock — example ncode extension (Node, no dependencies)
...
This copies the manifest + script into `$NCODE_HOME/extensions/clock/`.
ncode picks it up the next time you launch the TUI.
...
ncode doesn't care which one you use; it just spawns whatever `exec`
